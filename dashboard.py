...
feature_dict = {
    ...
    'GDP': 'Kapasitas Makroekonomi (GDP)',
    'Population': 'Estimasi Populasi (Population)',
    'Industrial_Index': 'Indeks Industri (Industrial_Index)'
}
...
feature_desc = {
    ...
    'GDP': 'Kekuatan ekonomi makro; daya beli berbanding lurus dengan serapan energi nasional.',
    'Population': 'Jumlah penduduk yang menentukan batas bawah beban dasar (Base-Load) residensial.',
    'Industrial_Index': 'Tingkat aktivitas manufaktur pabrik yang mengkonsumsi listrik bervoltase tinggi.'
}
...
with tab1:
    ...
    with col_b:
        ...
        try:
            st.image("output_xai.png", use_container_width=True)
        except Exception:
            st.info("No output_xai.png found.")
...
with tab2:
    ...
    with col2:
        # Mengubah Slider menjadi Number Input agar bisa diketik langsung
    ...
    if st.button("Jalankan Prediksi AI 🚀", use_container_width=True):
        ...
        custom_data = pd.DataFrame({
            ...
            'Lag_7': [last_known_data['Lag_7']], 
            'Lag_30': [last_known_data['Lag_30']],
            'Rolling_7': [last_known_data['Rolling_7']],
            'GDP': [last_known_data['GDP']],
            'Population': [last_known_data['Population']],
            'Industrial_Index': [last_known_data['Industrial_Index']]
        }).astype(float)
        ...
        feature_impacts = list(zip(iso_features, shap_vals))
